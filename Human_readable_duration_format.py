...
def format_duration(seconds):
    if seconds <= 0:
        return 'now'
    dividers = {'year': 31536000, 'day': 86400, 'hour': 3600, 'minute': 60, 'second': 1}
    res_list = []
    for k, v in dividers.items():
        if seconds // v > 0:
            res_list.append(f'{seconds // v} {k}{"s" if seconds // v > 1 else ""}')
        seconds %= v
    res_str = ''
    for i, time in enumerate(res_list):
        res_str += time
        if len(res_list) >= 2:
            if i < len(res_list) - 2:
                res_str += ', '
            elif i == len(res_list) - 2:
                res_str += ' and '
    return res_str


if __name__ == '__main__':
    assert format_duration(seconds=3662) == "1 hour, 1 minute and 2 seconds"